...
class s3_manager:
    ...
    def download_pipeline(self):
        if isfile('model/xgb_pipeline.pkl'):
            ...
            try:
                ...
                print("xgb_pipeline Downloaded")
            except:
                raise SamsResourceError(f"Pipeline is not present in {bucket_name.name}")
        
        if isdir('object_score'):
            print("Object score models downloaded")
        else:
            bucket_name ='sams-models'
            if not isdir('objectScore'):
                mkdir('objectScore')
            try:
                self.s3.meta.client.download_file(bucket_name,'xgb_obj_model.pkl','objectScore/xgb_obj_model.pkl')
                print("xgb_obj_model Downloaded")
                self.s3.meta.client.download_file(bucket_name,'dict_transformer_ht.pickle','objectScore/dict_transformer_ht.pickle')
                print("dict_transformer_ht Downloaded")
                self.s3.meta.client.download_file(bucket_name,'dict_transformer_hp.pickle','objectScore/dict_transformer_hp.pickle')
                print("dict_transformer_hp Downloaded")
                self.s3.meta.client.download_file(bucket_name,'dict_transformer_bt.pickle','objectScore/dict_transformer_bt.pickle')
                print("dict_transformer_bt Downloaded")
                self.s3.meta.client.download_file(bucket_name,'dict_transformer_bp.pickle','objectScore/dict_transformer_bp.pickle')
                print("dict_transformer_bp Downloaded")
                self.s3.meta.client.download_file(bucket_name,'count_vectorizer_h.pickle','objectScore/count_vectorizer_h.pickle')
                print("count_vectorizer_h Downloaded")
                self.s3.meta.client.download_file(bucket_name,'count_vectorizer_b.pickle','objectScore/count_vectorizer_b.pickle')
                print("count_vectorizer_b Downloaded")
            except:
                raise SamsResourceError(f"Object score models not bresent in {bucket_name.name}")
    ...
    def download_data(self):
        if isfile('data/Domain_info.csv'):
            ...
            bucket_name = 'sam-data'
            if not isdir('data'):
                mkdir('data')
            try:
                self.s3.meta.client.download_file(bucket_name,'Domain_info.csv','data/Domain_info.csv')
            except:
                raise SamsResourceError(f"Domain_info.csv is not present in {bucket_name.name}")
        
        if isfile('data/source_scores.csv'):
            print("Data Already Imported")
        else:
            bucket_name = 'sam-data'
            try:
                self.s3.meta.client.download_file(bucket_name,'source_scores.csv','data/source_scores.csv')
            except:
                raise SamsResourceError(f"source_scores.csv is not present in {bucket_name.name}")
