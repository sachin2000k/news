...
class FakeScore:
    def __init__(self):
        ...
        with open('fake_tokenizer.pickle', 'rb') as handle:
            self.tokenzs = pickle.load(handle)
        with open('model_in_json2.json','r') as f:
            self.model_json = json.load(f)
            #print(self.model_json)
            
        self.l_model = model_from_json(self.model_json)
        self.l_model.load_weights('model_weights2.h5')
        self.bigrams = Phraser.load("bigrams")
        self.trigrams = Phraser.load("trigrams")
        self.fake_score = 0
